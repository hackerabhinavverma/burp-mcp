...
public class HttpUtils {
    ...
    /**
     * Build an HttpRequest from a saved-request id OR a raw payload, so the
     * send-to-* tools can accept either form. Returns null when neither path
     * is satisfied (caller surfaces the error).
     *
     * Saved-id path: requires args.containsKey("id"); looks up by id in {@code model}.
     * Raw HTTP/1.1 path: requires data, host, port, secure.
     */
    public static HttpRequest buildHttpRequestFromArgsOrSaved(
            Map<String, Object> args,
            SavedRequestListModel model,
            boolean crlfReplace) {
        if (args.containsKey("id") && args.get("id") != null) {
            int requestId;
            Object idObj = args.get("id");
            if (idObj instanceof Number) {
                requestId = ((Number) idObj).intValue();
            } else {
                requestId = Integer.parseInt(idObj.toString());
            }
            for (int i = 0; i < model.getRowCount(); i++) {
                SavedRequestListModel.RequestEntry e = model.getEntry(i);
                if (e.getId() == requestId) {
                    HttpRequestResponse rr = e.getRequestResponse();
                    return rr.request();
                }
            }
            throw new IllegalArgumentException("Saved request id not found: " + requestId);
        }
        if (args.containsKey("data") && args.containsKey("host") && args.containsKey("port") && args.containsKey("secure")) {
            return buildHttp1Request(args, crlfReplace, false);
        }
        throw new IllegalArgumentException("Provide either 'id' (saved request) or 'data', 'host', 'port', 'secure' (raw HTTP/1.1 request)");
    }
