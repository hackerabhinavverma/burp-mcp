...
/** Pushes a saved or freshly built HTTP/1.1 request into Burp Intruder. */
public class SendToIntruderTool {

    private final MontoyaApi api;
    private final BurpMCP burpMCP;
    private final SavedRequestListModel savedRequestListModel;

    public SendToIntruderTool(MontoyaApi api, BurpMCP burpMCP, SavedRequestListModel savedRequestListModel) {
        this.api = api;
    ...
    public SyncToolSpecification createToolSpecification() {
        String schema = """
        {
            ...
            "properties": {
                ...
                "tabName": {"type": "string", "description": "Optional tab name in Intruder"}
            }
        }
        """;
        ...
        Tool tool = new Tool(
                "send-to-intruder",
                "Sends a request to Burp Intruder. Provide either a saved-request id or raw HTTP/1.1 fields.",
                schema);
    ...
    private CallToolResult handleToolCall(McpSyncServerExchange exchange, Map<String, Object> args) {
        ...
        burpMCP.writeToServerLog("To server", client, "send-to-intruder",
        ...
        try {
            ...
            result = new CallToolResult(Collections.singletonList(
                    new TextContent("Sent to Intruder" + (tabName != null ? " (tab: " + tabName + ")" : ""))), false);
        } catch (Exception e) {
            result = new CallToolResult(Collections.singletonList(
                    new TextContent("ERROR: send-to-intruder failed: " + (e.getMessage() != null ? e.getMessage() : e.getClass().getSimpleName()))), true);
        }
        burpMCP.writeToServerLog("To client", client, "send-to-intruder", result.toString());
        return result;
    }
}
